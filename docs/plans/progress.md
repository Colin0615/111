...
2. **AI 引擎**: Claude Haiku 4.5 为主 (低成本日常分析), Sonnet 关键决策, ~$5-35/月
3. **架构**: 微服务 + Celery 异步任务，便于独立扩展各模块
4. **数据库**: PostgreSQL (结构化数据) + Redis (缓存/队列)
5. **部署**: 先本地电脑 → 验证盈利后上 VPS/云
6. **安全策略**: Paper Trading 先行，小额起步，硬性止损
7. **初始资金**: $50 USDC.e
8. **市场覆盖**: 分层全覆盖 — 加密(40%) > 政治(30%) > 经济/科技(20%) > 储备(10%)
9. **月度预算**: 入门级 ~$35/月
10. **自动化程度**: AI 建议 + 人工确认 (初期)
11. **新闻源**: 先全免费 (NewsAPI免费层 + RSS + Reddit)
12. **核心策略**: 优先找"信息不对称"市场 (新上线/冷门/事件驱动)
...
| 单笔最大 | $5 (10%) | 绝不超过 |
| 单笔最小 | $2 | 手续费考虑 |
| 最低 Edge | 8% | 小资金要求更高 edge |
| 同时持仓 | 3-5 个市场 | 分散风险 |
| 每日最多 | 3 笔 | 专注高 edge |
| 止损线 | 总亏 $10 (20%) 暂停 | 保护本金 |
| 订单类型 | 优先限价单 | 赚 maker 返佣 |

### 待确认问题
- [ ] 确定所在地区的合规要求（美国需用 Polymarket US/CFTC）
- [ ] 是否已有 Polygon 钱包和 USDC.e？
